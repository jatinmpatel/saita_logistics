...
if(!function_exists('mainMenu')) {
    function mainMenu(){
        ...
        if(count($permission)==0){
            $permission = \DB::table('role_permission')->select('module_route')->where('role_id',Auth::user()->intRole)->get();
        }
...
if(!function_exists('checkAccess')) {
    function checkAccess($module,$accessPermission){
        ...
        if(!isset($permission->id)){
            $permission = \DB::table('role_permission')->select('id')->where('module_route',$module)->where($accessPermission,0)->where('role_id',Auth::user()->intRole)->first();
            if(isset($permission->id)){
                $access = true;
            }
        }else{
